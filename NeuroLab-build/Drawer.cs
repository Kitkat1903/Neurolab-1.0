...
namespace Neuron{
    public enum typeView{Line,Rectangle}
    public class Drawer : UserControl{
        ...
        System.Drawing.Point rightButtonFirstPoint,rightButtonLastPoint,
            leftButtonFirstPoint,leftButtonLastPoint;
        ...
        public class GraphicInfo{
            public string name;
            public Brush color;
            public GraphicInfo(string Name,Brush brush){name=Name;color=brush;}
        }
        public void Clear(){functions.Clear();needCreateBuffer=true;}
        ...
        public List<DrawerFunction> Functions{get{return functions;}}
        public PointF GraphicBounds{
            get{return graphicBounds;}
            set{
                graphicBoundsStack[stackValue=0]=graphicBounds=value;
                EnableCreatingBuffer(); needRedrawAxes=true;
            }
        }
        public PointF[] GetPoints(int index){return functions[index].RealPoints.ToArray();}
        ...
        private void CreateBitmapBuffer(){
            backColorBrush = new LinearGradientBrush(
                new Rectangle(0,0,Width,Height),
                Color.Green,Color.LightBlue,LinearGradientMode.Vertical);
            ...
            needRedrawAxes = true; Redraw();
        }
        void Drawer_Resize(object sender, EventArgs e){CreateBitmapBuffer();}
        ...
        void Drawer_MouseMove(object sender,MouseEventArgs e){
            if (enableZoom && rightButtonDown){
                ...
                tempBitmap=(Bitmap)memBitmap.Clone();
                tempGraphics=Graphics.FromImage(tempBitmap);
                tempGraphics.DrawRectangle(Pens.Brown,rightButtonFirstPoint.X,rightButtonFirstPoint.Y,
                    rightButtonLastPoint.X-rightButtonFirstPoint.X,
                    rightButtonLastPoint.Y-rightButtonFirstPoint.Y);
        ...
        private void SetClipRectangle(bool flag){
            if (flag) memGraphics.Clip = new Region(
                new RectangleF(offsetX, offsetY, Width-2*offsetX, Height-2*offsetY));
        ...
        void Drawer_MouseUp(object sender, MouseEventArgs e){
            if (enableZoom && e.Button==MouseButtons.Right){
                rightButtonDown=false; useTempGraphics=false;
                if (rightButtonFirstPoint.X > rightButtonLastPoint.X){
                    if (stackValue--==0) stackValue=0;
                    CreateBuffer(
                        new PointF(graphicBoundsStack[stackValue].X, graphicBoundsStack[stackValue].Y));
                }else{
            ...
            try{
                ...
                for (int k=0; k<functions.Count; k++){
                    ...
                    if(functions[k].type==typeView.Line)
                        memGraphics.DrawLines(new Pen(
                            functions[k].GraphicColor,2),functions[k].ScreenPoints.ToArray());
                    else if(functions[k].type==typeView.Rectangle)
                        memGraphics.DrawRectangles(new Pen(
                            functions[k].GraphicColor,2), GetRectangle(k));
                }
            } catch (Exception e){}
        }
        void DrawAxes(System.Drawing.Point partitionCount){
            ...
            float xReal=graphicBounds.X;
            float xRealAddition=(graphicBounds.Y-graphicBounds.X)/partitionCount.X;
            float yReal=maxFunc;
            float yRealAddition=(maxFunc-minFunc)/partitionCount.Y;
            ...
            for (int i=0; i<partitionCount.X+1; i++, x+=xAddition, xReal+=xRealAddition){
                ...
                axesGraphics.DrawString(xReal.ToString("0.00"),new Font("Arial",8),Brushes.Black,
                    new PointF(x, Height-offsetY+20), strFormat);
            }
            for (int i=0 ; i<partitionCount.Y+1; i++, y+=yAddition, yReal-=yRealAddition){
                ...
                axesGraphics.DrawString(yReal.ToString("0.00"),new Font("Arial",8),Brushes.Black,
                    new PointF(offsetX-20, y), strFormat);
            }
            axesGraphics.DrawRectangle(new Pen(Color.Black,2),
                offsetX, offsetY, Width-2*offsetX, Height-2*offsetY);
            DrawGraphicInfo();
        }
        private void DrawGraphicInfo(){
            ...
            for (int i = 0; i < functions.Count; i++){
                ...
                axesGraphics.DrawString(functions[i].GraphicInfo.name, new Font("Times New Roman",10),
                    Brushes.Black, new PointF(offsetX + offset + 20, offsetY / 2), strFormat);
                axesGraphics.FillRectangle(functions[i].GraphicInfo.color,
                    new RectangleF(offsetX+offset, offsetY/2-size.Height/2, size.Height, size.Height));
                offset+=size.Width+30;
            }
        }
        public override void Refresh(){Drawer_Paint(this, paintArgs);}
        public void Redraw(){

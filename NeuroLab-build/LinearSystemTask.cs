...
namespace Neuron{
    public class LinearSystemTask: NeuronNet{
        ...
        public override int NeuronGroupsCount{get{return NeuronGroups.Count;}}
        public override int OutputsCount{
            get{return LastNeuronGroup.Neurons.Count;}
            set{if (value>0) InputsCount=value;}
        }
        ...
        public int InitInputs(Matrix X){
            ...
            for (int i=0; i<X.Height; i++) Inputs[i].value=X[0,i];
            return 1;
        }
        public int InitTask(Matrix A,Matrix B){
            ...
            InputsCount=A.Height; OutputsCount=A.Height;
            InitNet(A,B); return 1;
        }
            ...
            for (l=0; l<NeuronGroupsCount; l++)
                for (i=0; i<NeuronGroups[l].Neurons.Count; i++){
                    ...
                    for (j = 0; j < NeuronGroups[l].Neurons[i].sinapses.Count; j++)
                        NeuronGroups[l].Neurons[i].sinapses[j].value = (l == 0) ? 
                            (A[i,j]/maxValue) : A.Transponation()[i,j]/maxValue;
                }
            return 1;
        }
            ...
            for (int i=1; i<NeuronGroups.Count; i++){
                for (int j=0; j<NeuronGroups[i].Neurons.Count; j++){
                    ...
                    for (int k = 0; k < NeuronGroups[i-1].Neurons.Count; k++){
                        NeuronGroups[i].Neurons[j].sinapses.Add(CurSin = 
                            new Sinaps(NeuronGroups[i-1].Neurons[k]));
        ...
        public Matrix GetOutput(){
            ...
            for (int i=0; i<Inputs.Count; i++) X[0,i]=Inputs[i].value;
            return X;
        }
        public int GetEpoches(){return epochNum;}
    }
}
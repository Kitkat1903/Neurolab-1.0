...
namespace Neuron{
    public partial class RegressionForm : Form{
        ...
        private void ChangeIntervals(object sender,EventArgs e){
            switch (type){
                case FunctionType.ONE_ARGUMENT_FUNCTION:
                    ...
                    break;
                case FunctionType.TWO_ARGUMENT_FUNCTION:
                    drawer3D.XBounds = new Microsoft.DirectX.Vector2(
                        Single.Parse(X0.Text), Single.Parse(XN.Text));
                    drawer3D.ZBounds = new Microsoft.DirectX.Vector2(
                        Single.Parse(Z0.Text), Single.Parse(ZN.Text));
                    drawer3D.EnableCreateBuffers();
                    drawer3D.RecreateGraphic();
                    break;
            }
        ...
        private void FilterChanged(object sender,EventArgs e){
            switch (type){
                case FunctionType.ONE_ARGUMENT_FUNCTION:
                    if (drawer2D.Functions.Count>0)
                        drawer2D.Functions[0].ShowGraphic=SourceFunction.Checked;
                    if (drawer2D.Functions.Count>1)
                        drawer2D.Functions[1].ShowGraphic=RandomFunction.Checked;
                    if (drawer2D.Functions.Count>2)
                        drawer2D.Functions[2].ShowGraphic=NetOutput.Checked;
                    drawer2D.Redraw();
                    break;
                case FunctionType.TWO_ARGUMENT_FUNCTION:
                    if (drawer3D.FunctionsCount>0)
                        drawer3D.Functions[0].ShowGraphic=SourceFunction.Checked;
                    if (drawer3D.FunctionsCount>1)
                        drawer3D.Functions[1].ShowGraphic=RandomFunction.Checked;
                    if (drawer3D.FunctionsCount>2)
                        drawer3D.Functions[2].ShowGraphic=NetOutput.Checked;
                    drawer3D.EnableCreateBuffers();
                    drawer3D.RecreateGraphic();
                    break;
            }

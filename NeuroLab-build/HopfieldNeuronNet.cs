...
namespace Neuron{
    public class HopfieldNeuronNet : NeuronNet{
        ...
        public override int NeuronGroupsCount{get{return NeuronGroups.Count;}set{}}
        public override void SetNeuronsCount(int group,int count){InputsCount = count;}
                ...
                for (int j=0; j<LastNeuronGroup.Neurons.Count; j++){
                    ...
                    /* if(i!=j){
                        for (int k=0; k<studyPairs.Count; k++){
                            LastNeuronGroup.Neurons[i].sinapses.Last().value
                            +=studyPairs[k].inputs[i]*studyPairs[k].inputs[j];
                        }
                    }*/
                }
            }            
        ...
        public override void Study(){SetSinapses();}
        private float NeuronFunction(float x){
            if (x>=0) return 1; return -1; //return (float)Math.Tanh(x);
        }
        ...
        public void Relax(ColorGrid.ColorGrid grid){
            ...
            for (int i=0; i<grid.Data.GetLength(0); i++){
                for (int j=0; j<grid.Data.GetLength(1); j++){
                    grid.Data[j, i].FloatValueAssociationMemory=
                        inputMatrix[i*grid.Data.GetLength(0)+j,0];
                }
            }
            grid.DrawGrid(); grid.RedrawData();
        }
    }
}
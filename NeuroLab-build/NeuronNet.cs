...
namespace Neuron{
    ...
    public class NeuronInput{
        ...
        public string Name;
        public NeuronInput(string Name){this.Name=Name;}
        public NeuronInput(float val){value=val;}
    }
    [Serializable]
    public class Sinaps{
    ...
    public enum ActivationFunction{
        LIMIT=0,LOGISTIC=1,TANGENS=2,RADIAL=3,
        GORBAN=4,LINEAR=5,RADIAL_VEC=6,SOFT_MAX=7
    }
    [Serializable]
    public class Neuron{
        public PointF Position;
        ...
        public float CalculateNET(){
            ...
            if (activationFunction==ActivationFunction.RADIAL_VEC){
                ...
                for (int i=0; i<sinapses.Count; i++){
                    buffer=(
            sinapses[i].frominput==null ? sinapses[i].fromNeuron.OUT : sinapses[i].frominput.value)-Centers[i];
                    NET+=buffer*buffer;
                }
                //NET = (float)Math.Sqrt(NET);
            }else{
                for (int i=0; i<sinapses.Count; i++)
                    NET+=sinapses[i].value*(
            sinapses[i].frominput==null ? sinapses[i].fromNeuron.OUT : sinapses[i].frominput.value);
            }
            return NET;
        }
        public float dE_dX(){
        ...
        public override string ToString(){return string.Format("{0|0.00}",OUT);}
    }
    [Serializable]
    public class NeuronGroup{
        ...
        public NeuronGroup(){Neurons.Add(new Neuron(this));}
    ...
    public class StudyPair{
        ...
        public override string ToString(){
            ...
            for (int i=0; i<inputs.Count; i++)
                str+=inputs[i].ToString("0.00")+"|";
            str=str.Remove(str.Length-1)+";";
            for (int i=0; i<quits.Count; i++)
                str+=quits[i].ToString("0.00")+"|";
            str=str.Remove(str.Length-1)+";";
            str+=string.Format("&{0}&#",name);
            for (int i=0; i<realQuits.Count; i++) str+=realQuits[i].ToString("0.00")+"|";
            return str;
        }
    ...
    public class NeuronNet{
        ...
        public NeuronNet(){InitializeParser();}
        ...
        public NeuronGroup LastNeuronGroup{get{return NeuronGroups[NeuronGroups.Count-1];}}
        public virtual int OutputsCount{
            get {return LastNeuronGroup.Neurons.Count;}
            set{
                if (!AccessChangeNet) return;
        ...
        public void NormalizeInputs(){
            ...
            for (int i=0; i<studyPairs.Count; i++, InputsSum=0){
                for (int j=0; j<studyPairs[i].inputs.Count; j++)
                    InputsSum+=(float)Math.Pow(studyPairs[i].inputs[j],2);
                InputsSum = (float)Math.Sqrt(InputsSum);
                for (int j=0; j<studyPairs[i].inputs.Count; j++)
                    studyPairs[i].inputs[j]/=InputsSum;
            }
        }
        ...
        public virtual void Study(){
            ...
            errors.Clear();
            minError=float.MaxValue; VariableN.value=EraCount;
            for (int i = 0; i < EraCount; i++){
                // VariableI.value = i; E = parser.Calculate();
            ...
            errorForm.Show(); errorForm.drawer.Redraw();
        }
...
//              GraphicsNeuron.StudyStatus.Value++;
        ...
        public float GetNetError(){
            ...
            for (int i=0; i<LastNeuronGroup.Neurons.Count; i++){
                for (int j = 0; j < StudyPairs.Count; j++){
                    result += (float)Math.Pow(Math.Pow(StudyPairs[j].quits[i],2)
                        -Math.Pow(StudyPairs[j].realQuits[i],2),2);
                }
            }
            return result / 2;
        }
        ...
        public virtual void LoadNextStudyPair(int studyPair){
            ...
            for (int i=0; i<inputs.Count; i++)
                inputs[i].value=StudyPairs[studyPair].inputs[i];
            for (int i=0; i<LastNeuronGroup.Neurons.Count; i++)
                LastNeuronGroup.Neurons[i].studyValue=StudyPairs[studyPair].quits[i];
            Calculate();
        ...
        public NeuronGroup GetNeuronsByRectangle(Rectangle rect){
            ...
            for (int i=0; i<NeuronGroups.Count; i++){
                for (int j=0; j<NeuronGroups[i].Neurons.Count; j++){
                    if (rect.Contains(new Point((int)NeuronGroups[i].Neurons[j].Position.X,
                        (int)NeuronGroups[i].Neurons[j].Position.Y)))

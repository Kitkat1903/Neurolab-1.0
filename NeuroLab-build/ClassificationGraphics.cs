    ...
    public partial class ClassificationGraphics : UserControl{
        public class ObjectClass{
            ...
            public static Brush[] brushes = {
                ...
                Brushes.Blue, Brushes.DarkGoldenrod, Brushes.Maroon,
                Brushes.DarkOrange, Brushes.DarkBlue, Brushes.Cornsilk,
                Brushes.Coral, Brushes.Black, Brushes.Azure
            };
            public static int currentID = 0;
            public static int singleValueID = 1;
            public ObjectClass(){id=currentID++; brush=brushes[id];}
            public void SetNewValueID(){ids.Add(singleValueID++);}
        }
        enum NetType{LINEAR,KOHONEN}
        ...
        private PointF ValueToScreen(PointF value){
            return new PointF(shiftLR.X+value.X*((float)(Width-shiftLR.X-shiftLR.Y)/(xBounds.Y-xBounds.X)),
                shiftTB.X+(yBounds.Y-value.Y)*((float)(Height-shiftTB.X-shiftTB.Y)/(yBounds.Y-yBounds.X)));
        }
        private PointF ScreenToValue(PointF screen){
            return new PointF(
                (screen.X-shiftLR.X)/((float)(Width-shiftLR.X-shiftLR.Y)/(xBounds.Y-xBounds.X)),
                yBounds.Y-(screen.Y-shiftTB.X)/((float)(Height-shiftTB.X-shiftTB.Y)/(yBounds.Y-yBounds.X)));
        }
        ...
        private void ReloadBitmap(){
            ...
            mainGraphics=CreateGraphics();
            DrawGraphics(); DrawObjects();
        }
        void ClassificationGraphics_Resize(object sender,EventArgs e){
            ReloadBitmap(); Refresh();
        }
        void ClassificationGraphics_Paint(object sender,PaintEventArgs e){
            mainGraphics.DrawImage(bitmap,new Point(0,0));            
        }
        private void DrawGraphics(){
            PointF addition=new PointF((
                xBounds.Y-xBounds.X)/gridLinesCount.X,(yBounds.Y-yBounds.X)/gridLinesCount.Y);
            PointF realAddition=new PointF(
                (float)(Width-shiftLR.X-shiftLR.Y)/gridLinesCount.X,
                (float)(Height-shiftTB.X-shiftTB.Y)/gridLinesCount.Y);
            PointF gridValues=new PointF(xBounds.X,yBounds.Y);
            grfx.Clear(Color.FromArgb(220,220,220));
            grfx.FillRectangle(Brushes.White,
                shiftLR.X,shiftTB.X,Width-shiftLR.Y-shiftLR.X,Height-shiftTB.Y-shiftTB.X);
            grfx.DrawRectangle(new Pen(Color.Black,2),
                shiftLR.X,shiftTB.X,Width-shiftLR.Y-shiftLR.X,Height-shiftTB.Y-shiftTB.X);
            for (int i=0; i<gridLinesCount.X+1; i++){
                grfx.DrawLine(gridPen,
                    shiftLR.X+i*realAddition.X,shiftTB.X,shiftLR.X+i*realAddition.X,Height-shiftTB.Y);
                grfx.DrawString(gridValues.X.ToString("0.00"),gridFont,Brushes.Black,
                    shiftLR.X+i*realAddition.X,Height-shiftTB.Y/2,strFormat);
                gridValues.X+=addition.X;
            }
            for (int i=0; i<gridLinesCount.Y+1; i++){
                grfx.DrawLine(gridPen,
                    shiftLR.X,shiftTB.X+i*realAddition.Y,Width-shiftLR.Y,shiftTB.X+i*realAddition.Y);
                grfx.DrawString(gridValues.Y.ToString("0.00"),gridFont,Brushes.Black,
                    shiftLR.X/2,shiftTB.Y+i*realAddition.Y,strFormat);
        ...
        private void DrawObject(PointF pt,Brush cl){
            ...
            grfx.FillEllipse(cl,
                pt.X-ObjectClass.size, pt.Y-ObjectClass.size, 2*ObjectClass.size, 2*ObjectClass.size);
        ...
        private void DrawObjects(){
            ...
            for (int i=0; i<classes.Count; i++){
                for (int j=0; j<classes[i].values.Count; j++){
                    ...
                    grfx.FillEllipse(classes[i].brush,
                        screenPoint.X-ObjectClass.size, screenPoint.Y-ObjectClass.size,
                        2*ObjectClass.size, 2*ObjectClass.size);
                    grfx.DrawString(classes[i].ids[j].ToString(),Font,Brushes.Black,
                        screenPoint.X+5,screenPoint.Y+5);
                }
            }
            for (int j=0; j<currentClass.values.Count; j++){
                screenPoint=ValueToScreen(currentClass.values[j]);
                grfx.FillEllipse(currentClassBrush,
                    screenPoint.X-ObjectClass.size, screenPoint.Y-ObjectClass.size,
                    2*ObjectClass.size, 2*ObjectClass.size);
                grfx.DrawString(currentClass.ids[j].ToString(),Font,Brushes.Black,
                    screenPoint.X+5,screenPoint.Y+5);
            }
            for (int i = 0; i < centerClasses.values.Count; i++){
                screenPoint = ValueToScreen(centerClasses.values[i]);
                grfx.DrawEllipse(new Pen(Color.Black,2),
                    screenPoint.X-ObjectClass.size-2, screenPoint.Y-ObjectClass.size-2,
                    2*ObjectClass.size+2, 2*ObjectClass.size+2);
            }
        }
        public override void Refresh(){mainGraphics.DrawImage(bitmap,new Point(0,0));}
        private void AddingClass(object sender, EventArgs e){
            ...
            DrawGraphics(); DrawObjects(); Refresh();
        }
        ...
        private void UpdateCenterClasses(){
            ...
            for (int i = 0; i < kohonenNet.LastNeuronGroup.Neurons.Count; i++){
                centerClasses.values.Add(
                    new PointF(
                        kohonenNet.LastNeuronGroup.Neurons[i].sinapses[0].value,
                        kohonenNet.LastNeuronGroup.Neurons[i].sinapses[1].value)
                    );
            }
            DrawGraphics();
            DrawObjects();
        ...
        private void DeleteAll(object sender, EventArgs e){
            ...
            currentClass=new ObjectClass();
            centerClasses.values.Clear();
            classes.Clear(); IsClassify.Checked=false;
            DrawGraphics(); DrawObjects(); Refresh();
        }
    }
}
    ...
    public class KohonenNeuronNet : NeuronNet{
        ...
        public override int OutputsCount{
            ...
            set{
                ...
                if (value>0) SetNeuronsCount(NeuronGroups.Count-1, value);
            }
        }
        ...
        public void LoadNextStudyPair(int studyPair, int time){
            ...
            for (int i=0; i<Inputs.Count; i++)
                Inputs[i].value=
                    StudyPairs[studyPair].inputs[i]*NormalizeFunction(time)
                    +(1.0f-NormalizeFunction(time))/((float)Math.Sqrt(InputsCount));            
            Calculate();            
        }
        ...
        public override float Calculate(){
            ...
            foreach (Neuron n in LastNeuronGroup.Neurons){
                ...
                for (int i=0; i<InputsCount; i++)
                    n.NET+=(float)Math.Pow(Inputs[i].value-n.sinapses[i].value,2);
                n.OUT=(float)Math.Exp(-n.NET*5);
            }
            foreach (Neuron n in LastNeuronGroup.Neurons){
                if (outValue<n.OUT){outValue=n.OUT; winner=n;}
            }
            winner.OUT=1;
            return 0;
        }

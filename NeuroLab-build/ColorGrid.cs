...
namespace ColorGrid{
    public partial class ColorGrid : UserControl{
        ...
        LinearGradientBrush brush=new LinearGradientBrush(new Rectangle(0,0,30,30),
            Color.DarkBlue,Color.Yellow,40);
        public ColorGrid(int width,int height){
            InitializeComponent();
            InitializeGrid(width , height);
            Resize += new EventHandler(ColorGrid_Resize);  
        }
        void ColorGrid_Resize(object sender,EventArgs e){RedrawGrid();RedrawData();}
        public GridItem[,] Data{get{return data;}}
        ...
        public Bitmap GetBitmap(){return memBitmap;}
        private void RedrawGrid(){
            ...
            brush = new LinearGradientBrush(GetItemRectangleByIndex(new Point(0,0)),Color.Yellow,
                Color.DarkBlue,40);
            DrawGrid();
        ...
        public void RedrawData(){
            for (int i=0; i<gridSize.Width; i++)
                for (int j=0; j<gridSize.Height; j++){
                    brush = new LinearGradientBrush(GetItemRectangleByIndex(
                        new Point(j,i)),Color.Yellow,Color.DarkBlue,40);
                    memGraphics.FillEllipse(data[j,i].value ? brush:Brushes.White,
                        GetItemRectangleByIndex(new Point(j,i)));
                }
            Refresh();
        }
        public void DrawGrid(){
            ...
            for (int i=0; i<gridSize.Width; i++)
                memGraphics.DrawLine(pen,new PointF(i*gridItemSize.Width,0),
                    new PointF(i*gridItemSize.Width,Height));
            for (int j=0; j<gridSize.Height; j++)
                memGraphics.DrawLine(pen,new PointF(0,j*gridItemSize.Height),
                    new PointF(Width,j*gridItemSize.Height));
        }
        private RectangleF GetItemRectangleByIndex(Point index){
            gridItemSize = new SizeF((float)Width/gridSize.Width,(float)Height/gridSize.Height);
            RectangleF rect = new RectangleF(gridItemSize.Width*index.X+2,
                gridItemSize.Height*index.Y+2,gridItemSize.Width-4,gridItemSize.Height-4);
            return rect;
        }
        private RectangleF GetItemRectangleByMousePosition(PointF mousePosition){
            gridItemSize = new SizeF((float)Width/gridSize.Width,(float)Height/gridSize.Height);
            return GetItemRectangleByIndex(new Point((int)(mousePosition.X/gridItemSize.Width),
                (int)(mousePosition.Y/gridItemSize.Height)));
        }
        private Point GetIndexByMousePosition(PointF mousePosition){
            gridItemSize = new SizeF((float)Width/gridSize.Width,(float)Height/gridSize.Height);
            return new Point((int)(mousePosition.X/gridItemSize.Width),
                (int)(mousePosition.Y/gridItemSize.Height));
        }
        protected override void OnPaint(PaintEventArgs e){e.Graphics.DrawImage(memBitmap,new Point());}
        public override void Refresh(){OnPaint(paintArgs);}
        protected override void OnMouseDown(MouseEventArgs e){
            ...
            brush = new LinearGradientBrush(GetItemRectangleByIndex(
                new Point(pt.X,pt.Y)),Color.Yellow,Color.DarkBlue,40);
            memGraphics.FillEllipse(data[pt.X,pt.Y].value ? 
                brush:Brushes.White, GetItemRectangleByMousePosition(new PointF(e.X,e.Y)));
            Refresh();            
        }
        ...
        protected override void OnMouseUp(MouseEventArgs e){mStatus.UpEvent(e);}
    }
    public class GridItem{
        public bool value;
        public float FloatValueClassification{
            get{if (value) return 1.0f; return 0.0f;}
            set{
        ...
        public float FloatValueAssociationMemory{
            get{if (value) return 1.0f; return -1.0f;}
            set{
                if (value > 0) this.value = true;
                else this.value = false;
            }
        }
        public GridItem(){value=false;}
    }

...
namespace _DGraphics{
    public class Coord{
        ...
        Coord(){x=y=z=phi=theta=0;radius=1;}
        Coord(float p1,float p2,float p3,bool IsNormal){
            if (IsNormal){
                x=p1;y=p2;z=p3;
                phi=0;theta=3.14f;radius=1;
            }else{
                phi=p1;theta=p2;radius=p3;
                x=y=z=0;
            }
        }
        Coord(float [] normal,bool empty){
            x=normal[0];y=normal[1];z=normal[2];
            phi=theta=0;radius=1;
        }
        Coord(float [] polar){
            phi=polar[0];theta=polar[1];radius=polar[2];
            x=y=z=0;
        }
        float [] ToNormal(){
        ...
        float [] ToPolar(){
            ...
            if (x>0 || z>=0) polar[0]=phi=(float)Math.Atan(z/x);
            else
                if (x<=0)
        ...
        public static void RotateNormalCoordinates(ref Vector3 Coords,float phi,float theta){
            ...
            tempTranslate.ToPolar(); tempTranslate.phi=phi;
            tempTranslate.theta=theta; tempTranslate.ToNormal();

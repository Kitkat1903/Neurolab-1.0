            ...
            try{
                if (IsSqlServerConnected=isSqlServer){
                    connection = new SqlConnection(string.Format(
                        "user ID={0};server={1};database={2};Trusted_Connection=yes;connection timeout=3",
                        UserID,Server,DatabaseName));
                    connection.Open();               
                }else{
                    oleConnection=new OleDbConnection(String.Format(
                        "provider=Microsoft.Jet.OLEDB.4.0;Data Source={0}",DatabaseName));
                    oleConnection.Open();
                }
            }catch (Exception){return false;}
            InitializeDataAdapter(); return true;
        }
        public void InitializeDataAdapter(){adapter=new SqlDataAdapter();oleAdapter=new OleDbDataAdapter();}
        public void FillTables(){
            dataSet = new DataSet();
            for (int i = 0; i < tables.Count; i++){
                DataTable table = new DataTable();
                adapter.SelectCommand.CommandText=string.Format("select * from [{0}];",tables[i]);
                adapter.Fill(table); dataSet.Tables.Add(table);
            }
        }

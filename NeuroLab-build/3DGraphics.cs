    ...
    public partial class Graphics3D : UserControl{
        public class Function3D{
            ...
            public static Color[] colors={
                Color.Green,Color.DarkOrange,Color.Yellow,Color.Violet,Color.Teal,Color.Snow};
            public Function3D(int id,GraphicFunction3D func){ID=id;GraphicFunction=func;}
            public void CreateMesh(Device device,int NumberFaces,int NumberVertices){
                if (GraphicMesh == null)
                    GraphicMesh = new Mesh(NumberFaces, NumberVertices, MeshFlags.Managed,
            ...
            public void AllocateMemory(int quadsCount){
                if (vertex==null){
                    ...
                    for (int i = 0; i < vertex.Length; i++)
                        vertex[i]=new CustomVertex.PositionNormalColored(
        ...
        public class GraphicParameters{
            ...
            public GraphicParameters(int stepCount){StepCount=stepCount;}
            public void CalculateParameters(Vector2 xBounds, Vector2 zBounds){
                ...
                ShiftX=StepX/2; ShiftZ=StepZ/2;
                QuadCountX=stepCount; QuadCountZ=stepCount;
                QuadCount=QuadCountX*QuadCountZ;
                TextureIncrementX=2.0f/QuadCountX; TextureIncrementZ=2.0f/QuadCountZ;
                NumberFaces=2*QuadCount; NumberVertices=3*NumberFaces;
                TexturePositionX=0; TexturePositionZ=2;
            }
        }
        Device device;
        ...
        Vector3 newRotate=new Vector3(0,0,0),oldRotate=new Vector3(Geometry.DegreeToRadian(-90f),
            Geometry.DegreeToRadian(90f),0);
        ...
        enum GraphicType{Diagramm=1,Graphic=2}
        public Color BackColorValue{
            get{return backColor;}
            set{backColor=value;Invalidate();}
        }
        public Function3D[] Functions{get{return functions;}}
        public Vector2 XBounds{
            get{return xBounds;}
            set{xBounds=value;CreateAxesLines();}
        }
        public Vector2 ZBounds{
            get{return zBounds;}
            set{zBounds=value;CreateAxesLines();}
        }
        public void RenewBounds(){CreateMesh();Invalidate();}
        public Graphics3D(){
            InitializeComponent();
        ...
        public int FunctionsCount{get{return functionsCount;}}
        ...
        public void SetDiagrammValues(float[,] values){
            ...
            CreateDiagramm(); Invalidate();
        }
        private void CreateDiagramm(){
            SizeF quadSize=new SizeF(
                (xRealBound.Y-xRealBound.X)/quadsCount.X,(zRealBound.Y-zRealBound.X)/quadsCount.Y);
            CustomVertex.PositionColored[] tmpVertex; Mesh tmpMesh;
            for(int i=0; i<quadsCount.X; i++){
                for (int j=0; j<quadsCount.Y; j++){
                    ...
                    tmpMesh=diagramm[i, j].Clone(diagramm[i,j].Options.Value,
                        CustomVertex.PositionColored.Format,device);
                    tmpVertex=(CustomVertex.PositionColored[])tmpMesh.VertexBuffer.Lock(0,
                        typeof(CustomVertex.PositionColored),LockFlags.None,tmpMesh.NumberVertices);
                    for (int vert=0; vert<tmpVertex.Length; vert++)
                        tmpVertex[vert].Color=Color.FromArgb((int)(diagrammValues[i,j]*255),
                            (int)((1.0f-diagrammValues[i,j])*255),0).ToArgb();
                    tmpMesh.VertexBuffer.Unlock();
                    diagramm[i,j].Dispose(); diagramm[i,j]=tmpMesh;
                    diagramm[i,j]=diagramm[i,j].Clone(diagramm[i,j].Options.Value,
                        CustomVertex.PositionNormalColored.Format,device);
        ...
        private void DrawDiagramm(){
            ...
            for (int i=0; i<quadsCount.X; i++){
                for (int j=0; j<quadsCount.Y; j++){
                    ...
                    device.Transform.World=Matrix.Scaling(new Vector3(scale,scale,scale))*Matrix.RotationY(
                        Geometry.DegreeToRadian(180f))*Matrix.Translation(new Vector3(quadSize.Width*i+
                        xRealBound.X+boxShift,diagrammValues[i,j]/2,quadSize.Height*j+zRealBound.X+boxShift));
        ...
        private void InitializeGraphics(){
            ...
            SetCamera(); CreateMesh(); CreateBoxTexture(10,10);
            //CreateAxesLines();
            Refresh();
        }
        public void SetCamera(){
            device.Transform.Projection=Matrix.PerspectiveFovLH(
                Geometry.DegreeToRadian(90f),Width/Height,1,100);
            cameraDirection=new Vector3(0,0,0);
        ...
        private void CreateMesh(){
            FillBuffers();
            for (int i=0; i<functionsCount; i++)
                functions[i].CreateMesh(device,parameters.NumberFaces,parameters.NumberVertices);
        }
        private void CreateAxesValues (){
            ...
            for (int i=0; i<valuesCount; i++, x+=incX)
                xValues[curr++]=Mesh.TextFromFont(device,new System.Drawing.Font("Times New Roman",2),
                    x.ToString("0.00"),0.2f,0.05f);
            curr=0;
            for (int i=0; i<valuesCount; i++, z+=incZ)
                zValues[curr++]=Mesh.TextFromFont(device,new System.Drawing.Font("Times New Roman",2),
                    z.ToString("0.00"),0.2f,0.05f);
            curr=0;
            for (int i=0; i<valuesCount; i++, y+=incY)
                yValues[curr++]=Mesh.TextFromFont(device,new System.Drawing.Font("Times New Roman",2),
                    y.ToString("0.00"),0.2f,0.05f);
        }
        public void CreateAxesLines(){
            Vector3 center=new Vector3(
                Math.Abs(xBounds.X)-Math.Abs(xBounds.Y),
                Math.Abs(yBounds.X)-Math.Abs(yBounds.Y),
                Math.Abs(zBounds.X)-Math.Abs(zBounds.Y));
            axesPoints[0]=new CustomVertex.PositionColored(
                center.X,center.Y,center.Z,Color.Red.ToArgb());
            axesPoints[1]=new CustomVertex.PositionColored(
                center.X,center.Y+100,center.Z,Color.Red.ToArgb());
            axesPoints[2]=new CustomVertex.PositionColored(
                center.X,center.Y,center.Z,Color.Green.ToArgb());
            axesPoints[3]=new CustomVertex.PositionColored(
                center.X,center.Y,center.Z+100,Color.Green.ToArgb());
            axesPoints[4]=new CustomVertex.PositionColored(
                center.X,center.Y,center.Z,Color.Blue.ToArgb());
            axesPoints[5]=new CustomVertex.PositionColored(
                center.X+100,center.Y,center.Z,Color.Blue.ToArgb());
        }
        private void CreateBoxTexture(int X,int Y){
            ...
            for (int i=0; i<Y; i++, y+=addY) graphics.DrawLine(pen,0,y,boxBitmap.Width,y);
        ...
        private void DrawBoundBox(){
            ...
            device.Transform.World=Matrix.Translation(0,yRealBound.X-boxShift,0)*Matrix.Scaling(
                new Vector3(scale,scale,scale))*Matrix.RotationY(Geometry.DegreeToRadian(180f));
            boundBox.DrawSubset(0);
            device.Transform.World=Matrix.RotationZ(Geometry.DegreeToRadian(90f))*Matrix.Translation(
                xRealBound.X-boxShift,0,0)*Matrix.Scaling(new Vector3(scale,scale,scale))*Matrix.RotationY(
                    Geometry.DegreeToRadian(180f));
            boundBox.DrawSubset(0);
            device.Transform.World=Matrix.RotationX(Geometry.DegreeToRadian(90f))*Matrix.Translation(
                0,0,zRealBound.X-boxShift)*Matrix.Scaling(new Vector3(scale,scale,scale))*Matrix.RotationY(
                    Geometry.DegreeToRadian(180f));
            boundBox.DrawSubset(0);
            device.RenderState.Lighting = true;
        }
        private void DrawAxesValues (){
            ...
            for ( float x=xRealBound.X; x<xRealBound.Y; x+=incX){
                device.Transform.World=Matrix.Scaling(
                    new Vector3(scale*0.4f,scale*0.4f,scale*0.4f))*Matrix.RotationY(
                        Geometry.DegreeToRadian(180f))*Matrix.Translation(
                            new Vector3(-x+0.5f,yRealBound.X-0.5f,-zRealBound.X+0.5f));
            ...
            for (float z=zRealBound.X; z<zRealBound.Y;z+=incZ){
                device.Transform.World=Matrix.Scaling(
                    new Vector3(scale*0.4f,scale*0.4f,scale*0.4f))*Matrix.RotationY(
                        Geometry.DegreeToRadian(180f))*Matrix.Translation(
                            new Vector3(xRealBound.X-0.5f,yRealBound.X-0.5f,-z+0.5f));
            ...
            for (float y=yRealBound.X; y<yRealBound.Y; y+=incY){
                device.Transform.World=Matrix.Scaling(
                    new Vector3(scale*0.4f,scale*0.4f,scale*0.4f))*Matrix.RotationY(
                        Geometry.DegreeToRadian(180f)) * Matrix.Translation(
                            new Vector3(-xRealBound.X+1.0f,y-0.5f,-zRealBound.X+0.5f));
        ...
        private Mesh Rectangle ( float width , float height ){
            ...
            curIndices[0]=0;curIndices[1]=1;curIndices[2]=2;
            curIndices[3]=0;curIndices[4]=2;curIndices[5]=3;
        ...
        protected override void OnPaint(PaintEventArgs e){
            ...
            device.Transform.World=Matrix.Scaling(
                new Vector3(scale,scale,scale))*Matrix.RotationY(Geometry.DegreeToRadian(180f));
            DrawFunctions(); DrawBoundBox();
            device.EndScene(); device.Present();
        }
        public void EnableCreateBuffers(){
            for (int i=0; i<functionsCount; i++) functions[i].NeedCreateBuffer = true;
        }
        private void FillBuffers(){
            ...
            for (int k=0; k<functionsCount; k++, CurrentIndex=CurrentVertex=0){
                ...
                for (
                    int z=0; z<parameters.QuadCountZ; z++,
                    zPos-=parameters.StepZ,
                    parameters.RealZPos-=realStepZ,
                    parameters.TexturePositionZ-=parameters.TextureIncrementZ
                ){
                    parameters.RealXPos = xRealBound.X;
                    xPos = xBounds.X;
                    for (int x=0; x<parameters.QuadCountX; x++,
                        xPos+=parameters.StepX, parameters.RealXPos+=realStepX,
                        CurrentIndex+=6, CurrentVertex+=4,
                        parameters.TexturePositionX+=parameters.TextureIncrementX
                    ){
                        ...
                        //left up
                        functions[k].vertex[CurrentVertex].X=parameters.RealXPos;
/*;new CustomVertex.PositionNormalColored(new Vector3(parameters.RealXPos,parameters.FuncValue,
 * parameters.RealZPos),new Vector3(),/*new Vector2(TexturePositionX,TexturePositionZ)*/
 //Function3D.colors[2*k].ToArgb());
                        ...
                        //right up
                        functions[k].vertex[CurrentVertex+1].X=parameters.RealXPos + realStepX;
/*new CustomVertex.PositionNormalColored(new Vector3(parameters.RealXPos+realStepX,
 * functions[k].GraphicFunction(xPos+parameters.StepX,zPos),parameters.RealZPos),new Vector3(),
 * /*new Vector2(TexturePositionX + TextureIncrementX , TexturePositionZ)*/
//Function3D.colors[2 * k + 1].ToArgb());
                        functions[k].vertex[CurrentVertex+1].Y=functions[k].GraphicFunction(
                            xPos+parameters.StepX,zPos);
                        ...
                        //left down
                        functions[k].vertex[CurrentVertex+2].X=parameters.RealXPos;
/*new CustomVertex.PositionNormalColored(new Vector3(parameters.RealXPos,functions[k].GraphicFunction(
 * xPos,zPos-parameters.StepZ),parameters.RealZPos-realStepZ),new Vector3(),
 * /*new Vector2(TexturePositionX , TexturePositionZ - TextureIncrementZ)*/
 //Function3D.colors[2*k+1].ToArgb());//*/
                        functions[k].vertex[CurrentVertex+2].Y=functions[k].GraphicFunction(
                            xPos,zPos-parameters.StepZ);
                        ...
                        //right down
                        functions[k].vertex[CurrentVertex+3].X=parameters.RealXPos+realStepX;
/*new CustomVertex.PositionNormalColored(new Vector3(parameters.RealXPos+realStepX,functions[k].
 * GraphicFunction(xPos+parameters.StepX,zPos-parameters.StepZ),parameters.RealZPos-realStepZ),
 * new Vector3(),/*new Vector2(TexturePositionX+TextureIncrementX,TexturePositionZ-TextureIncrementZ)*/
 //Function3D.colors[2*k].ToArgb());//*/
                        functions[k].vertex[CurrentVertex+3].Y=functions[k].GraphicFunction(
                            xPos+parameters.StepX,zPos-parameters.StepZ);
        ...
        void Graphics3D_MouseWheel(object sender,System.Windows.Forms.MouseEventArgs e){
            if (e.Delta>0){scale+=0.05f;}
            else{scale-=0.05f;}
            Invalidate();
        }
        public override void Refresh(){OnPaint(paintEventArgs);}
        ...
        private void Graphics3D_Resize(object sender,EventArgs e){
            EnableCreateBuffers(); RecreateMeshes();
            InitializeGraphics();
